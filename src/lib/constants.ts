...
export const REGIONS = [
  ...
  { fo: 'Вне федеральных округов', color: '#ef4444', list: ['Донецкая НР', 'Луганская НР', 'Запорожская область', 'Херсонская область'] },
];

export const ALL_REGIONS = REGIONS.flatMap(g => g.list);

export function rfactor(regionName: string): number {
  let h = 0;
