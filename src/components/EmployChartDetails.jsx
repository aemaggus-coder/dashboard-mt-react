import { useScaledData } from '../hooks/useScaledData';
import { BASE } from '../lib/constants';

export default function EmployChartDetails() {
  const scaledData = useScaledData(BASE.employ, ['working', 'notWorking']);
  const { labels, working, notWorking } = scaledData;

  const rows = labels.map((label, i) => {
    const w = working[i];
    const nw = notWorking[i];
    const total = w + nw;
    return [
      label,
      (w * 1000).toLocaleString('ru-RU'),
      (nw * 1000).toLocaleString('ru-RU'),
      Math.round((w / total) * 100) + '%',
    ];
  });

  return (
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr style={{ borderBottom: '2px solid rgba(255,255,255,.09)' }}>
          <th style={{ textAlign: 'left', padding: '10px 16px', color: 'var(--text-2)', fontSize: '12px', fontWeight: '700' }}>
            Группа
          </th>
          <th style={{ textAlign: 'right', padding: '10px 16px', color: 'var(--text-2)', fontSize: '12px', fontWeight: '700' }}>
            Работают
          </th>
          <th style={{ textAlign: 'right', padding: '10px 16px', color: 'var(--text-2)', fontSize: '12px', fontWeight: '700' }}>
            Не работают
          </th>
          <th style={{ textAlign: 'right', padding: '10px 16px', color: 'var(--text-2)', fontSize: '12px', fontWeight: '700' }}>
            Занятость
          </th>
        </tr>
      </thead>
      <tbody>
        {rows.map((row, idx) => (
          <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,.06)' }}>
            <td style={{ padding: '12px 16px', color: 'var(--text)' }}>{row[0]}</td>
            <td style={{ padding: '12px 16px', textAlign: 'right', color: '#10b981', fontFamily: 'var(--mono)', fontWeight: '600' }}>
              {row[1]}
            </td>
            <td style={{ padding: '12px 16px', textAlign: 'right', color: '#ef4444', fontFamily: 'var(--mono)', fontWeight: '600' }}>
              {row[2]}
            </td>
            <td style={{ padding: '12px 16px', textAlign: 'right', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: '600' }}>
              {row[3]}
            </td>
          </tr>
        ))}
      </tbody>
    </table>
  );
}
